function [ imsize, micronsperpixel ] = conecountingfunc( fname, pname,outpath, lutData )
...
%Read in image and select ROI
prompt = 'Do you wish to draw an ROI within the selection? (Y/N)';
res = input(prompt, 's');
yes = 'y';
if strcmpi(yes,res)
    temp = imread(fullfile(pname, fname));
    imageA = imcrop(temp);
else
...
clipped_coords=coordclip_npoly([Y X],[2 max(Y)-1],[2 max(X)-1]);
...
% user added them...
manual_mod_cones=coordclip_npoly(manual_mod_cones,[2 max(X)-1],[2 max(Y)-1]);
