...
% Default to reliability of 4.
[ density_per_annulus um_drp_sizes est_spacing] = calculate_DRP( coords , [clip_start_end(2,:); clip_start_end(1,:) ] , um_per_pix, density, 4 );

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Output List Formatting %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
statout=[numcells density mean_nn_dist total_coord_area*(1000^2) mean_cellarea length(cellarea) total_cell_area est_spacing];
...
%     disp(statoutfname);
    ...
    fprintf(fid,['"Filename","Number of Cells","Cell Density (cells/mm^2)","Mean N-N (um)",'...
                 ...
                 '"Number of Bounded Cells","Total Voronoi Area (um^2)","DRP Spacing (um)"\n']);
    fprintf(fid,['"' fname '",']); % 1st/2nd column- filename/Detection Type
    fclose(fid);
    dlmwrite(statoutfname,statout,'-append'); % 2-6 columns- data
